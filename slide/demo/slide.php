...
======================================= -->
		<div id="featured"> 
			...
			<?php
				$sql= "select codigo,imagem FROM fotos_slide_show  where(fotos_slide_show.empresa=3)";//oficialestandes
				...
				if ($row = mysql_fetch_assoc($result)){				
					...
					$result_fotos = mysql_query($sql_fotos, $link);
						...
						$imagem=$row["imagem"];
			?>
			<a href=""><img width="1000px" height="330px" src="http://www.oficialestandes.com.br/upload/imagens/fotos_slide_show/<?php echo $imagem;?>" data-caption="#htmlCaption" /></a>
			<?	}?>
			<?php
				$sql= "select codigo,imagem FROM fotos_slide_show  where(fotos_slide_show.empresa=1)";//comunicacao
				...
				if ($row = mysql_fetch_assoc($result)){				
					...
					$result_fotos = mysql_query($sql_fotos, $link);
						...
						$imagem=$row["imagem"];
			?>
			<a href=""><img width="1000px" height="330px" src="../../comunicacao/upload/imagens/fotos_slide_show/<?php echo $imagem;?>"/></a>
			<?	}?>
			<?php
				$sql= "select codigo,imagem FROM fotos_slide_show  where(fotos_slide_show.empresa=2)";//grafica
				$result=mysql_query($sql, $link);
				$i=0;
				if ($row = mysql_fetch_assoc($result)){				
					$descricao=$row["descricao"];
					...
					$result_fotos = mysql_query($sql_fotos, $link);
						...
						$imagem=$row["imagem"];
			?>
			<a href=""><img width="1000px" height="330px" src="../../grafica/upload/imagens/fotos_slide_show/<?php echo $imagem;?>"  /></a>
			<?	}?>
